...
function copyDir(name = '') {
    fs.readdir(path.join(__dirname, 'assets', name), { withFileTypes: true }, (err, files) => {
      ...
      files.forEach(file => {
        if (file.isFile()) {
          fs.copyFile(path.join(__dirname, 'assets', name, file.name), path.join(__dirname, 'project-dist', 'assets', name, file.name), 
          err => {if (err) throw err})
        } else {
          fs.mkdir(path.join(__dirname, 'project-dist', 'assets', file.name), { recursive: true }, err => {if (err) throw err})
          copyDir(file.name)
        }
      })
    })
...
console.log('Спасибо за перепроверку!')